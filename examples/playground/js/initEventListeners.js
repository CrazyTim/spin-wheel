...
for (const g of inputGroups) {
  ...
  header.innerHTML =
    '<span class="prop-name"></span><span class="prop-value"></span>';
...
document
  ...
  .addEventListener("click", exportWheelAsJson);

function initRange(g) {
  g.querySelector("input").addEventListener("input", () => {
    const val =
      parseFloat(g.querySelector("input").value) * (g.dataset.multiplier ?? 1);
...
function initTextboxOrColor(g) {
  ...
  const initialVal = fixThreeCharHexColor(
    // Color input requires a 6 decimal hex
